...
const calcDaysOffCount = (totalDays: number) => {
  ...
  if (formData.value.unpaidDays > 0 &&  formData.value.paidDays <= 0) {
    return Math.floor( 30 / 365  * (totalDays -  formData.value.unpaidDays ) );
  }

  // if paid => 30 / 365  * TOTAL SERVICE DAYS - PAID DAYS
  if (formData.value.unpaidDays <= 0 &&  formData.value.paidDays > 0) {
    return Math.floor( 30 / 365  * totalDays -  formData.value.paidDays);
  }

  // if (paid & unpaid) => 30 / 365  * ( TOTAL SERVICE DAYS - UNPAID DAYS ) - PAID DAYS
  if(formData.value.unpaidDays > 0 && formData.value.paidDays > 0) {
    return Math.floor( 30 / 365  * ( totalDays - formData.value.unpaidDays ) - formData.value.paidDays )
  }

  //if not (paid or unpaid) 30  / 365 * TOTAL SERVICE DAYS
  return Math.floor(30  / 365 * totalDays );
}
...
const calcEndDaysCount = (totalDays: number) => {
  ...
  const afterFiveYearsTotal = afterFiveYearsCount * 26;
...
watchEffect(() => {
  ...
  if(formData.value.salary) {
    formData.value.salary = toEnglishDigits(formData.value.salary);
  }
  ...
  if(formData.value.salary > 0 && total.value.offDaysCount > 0) {
    dayRate = (formData.value.salary / 26).toFixed(2);
...
const storeData = async () => {
  const requestData = {
    ...
    meta: {
      ...
      salary: formData.value.salary + ' ' + t('kw'),
      ...
      serviceDaysCount: total.value.serviceDaysCount + ' ' + t('day'),
      offDaysCount: total.value.offDaysCount + ' ' + t('day'),
      endDaysCount: total.value.endDaysCount + ' ' + t('day'),
      dayRate: total.value.dayRate  + ' ' + t('kw'),
      endDaysTotal: total.value.endDaysTotal  + ' ' + t('kw'),
      offDaysTotal: total.value.offDaysTotal  + ' ' + t('kw'),
    }
  }
