...
const calcDaysOffCount = (totalDays: number) => {
  ...
  const dayRateAmount = 30;

  // if unpaid => 30 / 365 * (TOTAL SERVICE DAYS - UNPAID DAYS)
  if (formData.value.unpaidDays > 0 &&  formData.value.paidDays <= 0) {
    return Math.floor( dayRateAmount / 365  * (totalDays -  formData.value.unpaidDays ) );
  }

  // if paid => 30 / 365  * TOTAL SERVICE DAYS - PAID DAYS
  if (formData.value.unpaidDays <= 0 &&  formData.value.paidDays > 0) {
    return Math.floor( dayRateAmount / 365  * totalDays -  formData.value.paidDays);
  }

  // if (paid & unpaid) => 30 / 365  * ( TOTAL SERVICE DAYS - UNPAID DAYS ) - PAID DAYS
  if(formData.value.unpaidDays > 0 && formData.value.paidDays > 0) {
    return Math.floor( dayRateAmount / 365  * ( totalDays - formData.value.unpaidDays ) - formData.value.paidDays )
  }

  //if not (paid or unpaid) 30  / 365 * TOTAL SERVICE DAYS
  return Math.floor(dayRateAmount  / 365 * totalDays );
}

const calcEndDaysCountOLD = (totalDays: number) => {
  const totalYears = Math.floor(totalDays / 365 );
  let endServiceDaysCount = 0;

  for(let year = 1; year <= totalYears; year++) {
    let daysCount = year <= 5 ? 21 : 30;
...
const calcEndDaysCount = (totalDays: number) => {
  const totalYears = Math.floor(totalDays / 365 );
  let firstFiveYearsCount = 0;
  let afterFiveYearsCount = 0;

  for(let year = 1; year <= totalYears; year++) {
    if(year >= 1 && year <= 5) {firstFiveYearsCount++;}

    if(year > 5) {afterFiveYearsCount++;}
  }

  const firstFiveYearsTotal = totalYears >= 15 ? firstFiveYearsCount * 30 : firstFiveYearsCount * 21;
  const afterFiveYearsTotal = afterFiveYearsCount * 30;
  return Math.floor(firstFiveYearsTotal + afterFiveYearsTotal);
}
...
const calcEndDaysTotal = (endServiceRate: number, totalDays: number) => {
  ...
  if(formData.value.serviceStatus === 3) {
    ...
    if(totalServiceDaysInYears < 1) {
      total = 0;
    }
  }

  if(formData.value.serviceStatus === 4) {

    if(totalServiceDaysInYears < 1) {
      total = 0;
    }

    if(totalServiceDaysInYears >= 1 && totalServiceDaysInYears < 3) {
      total = endServiceRate / 3;
    }
            ...
            <div class="calc-list-inner">
              <div class="calc-subtotal-list">
                <div class="calc-subtotal-list-accordion">
                  ...
                  <div class="sub-list-item">
                    ...
                    <span class="sub-item-space"></span>
                    <span class="sub-item-value">{{ formData.salary < 0 ? 0 : formData.salary }} {{ $t("AED") }}</span>
                  </div>
                  <div class="sub-list-item">
              ...
              <div class="calc-subtotal-list total-list">
                ...
                <div class="sub-list-item total">
                  ...
                  <span class="sub-item-value">{{total.dayRate ? total.dayRate : 0}} {{$t("AED")}}</span>
                </div>

                <div class="sub-list-item total">
                  <span class="sub-item-title">{{ $t("offDaysTotal") }}</span>
                  <span class="sub-item-value">{{total.offDaysTotal}} {{$t("AED")}}</span>
                </div>

                <div class="sub-list-item total">
                  <span class="sub-item-title">{{ $t("endDaysTotal") }}</span>
                  <span class="sub-item-value">{{total.endDaysTotal}} {{$t("AED")}}</span>
                </div>
              </div>
